...
A window titled **Pose Tracking** shows the annotated camera feed. Press `q` to quit.
...
- **Model**: swap `yolov8m-pose.pt` for a faster (`yolov8n-pose.pt`) or more accurate (`yolov8x-pose.pt`) variant.
...
2. Each frame goes through YOLOv8 pose to get keypoints.
